...
namespace ShoppingPlatform.Repositories
{
    public interface IProductRepository
    {
        ...
        Task<(List<ProductListItemDto> items, long total)> QueryAsync(string? q, string? category, string? subCategory,
            decimal? minPrice, decimal? maxPrice,List<string>? fabric , int page, int pageSize, string? sort,string? country, List<string>? color, List<string>? sizes);
        ...
        Task<bool> DecrementStockAsync(string productObjectId, string size, string currency, int quantity);
